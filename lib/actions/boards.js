...
export async function importCards(boardId, defaultColumnId, cards) {
  ...
  const normalizedCards = cards.map((card, index) => {
    ...
    const requestedColumnTitle = String(card?.column || card?.columnTitle || '').trim().toLowerCase()
    const requestedColumn = requestedColumnTitle ? columnByTitle.get(requestedColumnTitle) : null
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const label of Array.from(new Set(normalizedCards.flatMap(card => card.labels)))) {
    ...
    for (const columnId of Array.from(new Set(normalizedCards.map(card => card.columnId)))) {
  ...
  revalidateBoard(boardId)
  refresh()
  return { count: normalizedCards.length }
}
