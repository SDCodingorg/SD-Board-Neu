...
async function assertCanRead(boardId, userId) {
  return getBoardAccess(boardId, userId)
}
...
export async function addBoardMember(boardId, email, role = 'editor') {
  const session = await getSession()
  await assertCanAdmin(boardId, session.user.id)

  const normalizedEmail = email?.trim()
  if (!normalizedEmail) throw new Error('Email ist erforderlich')

  const user = await prisma.user.findFirst({
    where: { email: { equals: normalizedEmail, mode: 'insensitive' } },
    select: { id: true },
  })
  if (!user) throw new Error('Dieser Account muss sich einmal mit Discord anmelden, bevor er eingeladen werden kann')

  if (user.id === session.user.id) throw new Error('Du bist bereits Mitglied')
