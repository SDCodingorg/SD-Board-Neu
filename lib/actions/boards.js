...
export async function saveBoardWhiteboard(boardId, data) {
  const session = await getSession()
  await assertCanWrite(boardId, session.user.id)

  const payload = data && typeof data === 'object' ? data : { version: 1, elements: [] }
  const elements = Array.isArray(payload.elements) ? payload.elements.slice(0, 500) : []
  const normalized = {
    version: 1,
    elements,
  }
  ...
  if (size > 2_000_000) throw new Error('Whiteboard ist zu gross zum Speichern')

  await prisma.boardWhiteboard.upsert({
    where: { boardId },
    create: { boardId, data: normalized },
